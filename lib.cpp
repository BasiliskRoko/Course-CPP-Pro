...
{
    auto finder1 = [value = first](auto a)
    { return value == atoi(a.cbegin()->c_str()); };
    printOn(ip_pool, finder1);
}

void filter(VectorVectorString &ip_pool, int first, int second)
{
    auto finder2 = [value1 = first, value2 = second](VectorString a)
    { return value1 == atoi(a.at(0).c_str()) && value2 == atoi(a.at(1).c_str()); };
...
void filter_any(VectorVectorString &ip_pool, int byte)
{
    ...
    {
        auto result = std::find_if(a.begin(), a.end(), [value](auto ip4)
                                   { return value == atoi(ip4.c_str()); });
        return result != a.end();
    };
